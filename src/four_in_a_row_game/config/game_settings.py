...
class GameSettings(BaseSettings):
    ...
    # === Board Configuration ===
    board_width: int = Field(default=7, description="Number of columns")
    board_height: int = Field(default=6, description="Number of rows")
    connect_length: int = Field(default=4, description="Number of pieces needed to win")

    # === Visual Configuration ===
    board_color: tuple[int, int, int] = (64, 128, 255)  # Blue
    player1_color: tuple[int, int, int] = (255, 64, 64)  # Red
    player2_color: tuple[int, int, int] = (255, 255, 64)  # Yellow
    background_color: tuple[int, int, int] = (32, 32, 32)  # Dark gray
    grid_color: tuple[int, int, int] = (128, 128, 128)  # Gray
    ...
    sound_volume: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Sound effects volume"
    )
    music_volume: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Sound effects volume"
    )
